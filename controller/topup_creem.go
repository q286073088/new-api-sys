...
func handleCheckoutCompleted(c *gin.Context, event *CreemWebhookEvent) {
	...
	listAmount, amountErr := common.WalletQuotaFromDecimalStrict(decimal.NewFromInt(int64(event.Object.Product.Price)).
		Mul(decimal.NewFromInt(int64(max(event.Object.Units, 1)))))
	if amountErr != nil || listAmount <= 0 || event.Object.Units < 0 || event.Object.Order.AmountPaid < 0 {
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem invalid payment amounts trade_no=%s", referenceId))
		c.AbortWithStatus(http.StatusBadRequest)
		return
	}
	err := model.RechargeCreem(referenceId, customerEmail, customerName, c.ClientIP(), &model.TopUpPayment{
		PaidAmount: int64(event.Object.Order.AmountPaid), ListAmount: int64(listAmount),
	})
	if err != nil {
