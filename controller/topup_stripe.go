...
func fulfillOrder(ctx context.Context, event stripe.Event, referenceId string, customerId string, callerIp string) error {
	if len(referenceId) == 0 {
		...
		return errors.New("missing Stripe order reference")
	}
	...
	paidAmount, paidErr := strconv.ParseInt(event.GetObjectValue("amount_total"), 10, 64)
	listAmount, listErr := strconv.ParseInt(event.GetObjectValue("amount_subtotal"), 10, 64)
	if paidErr != nil || listErr != nil || paidAmount < 0 || listAmount <= 0 {
		logger.LogError(ctx, fmt.Sprintf("Stripe invalid payment amounts trade_no=%s", referenceId))
		return errors.New("invalid Stripe payment amounts")
	}
	err := model.Recharge(referenceId, customerId, callerIp, &model.TopUpPayment{PaidAmount: paidAmount, ListAmount: listAmount})
	if err != nil {
