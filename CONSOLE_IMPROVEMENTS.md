...
| 功能 | 入口与行为 |
| --- | --- |
| 公告弹框 | 用户进入控制台后只弹发布日期最新的一条公告，最新公告已读时不补弹旧公告。方角弹框显示“系统公告”标题、正文、附加信息、发布时间及“知道了”按钮。没有有效公告条目时兼容首页 Notice；已展示记录保存到账号，重新登录或换设备后读取同一记录。 |
...
### 2026-09-14 最新公告与弹框外观

先从非空公告中选择发布日期最新的一条，再检查该账号是否已经看过。发布日期相同或都缺失时优先数值较大的 ID，无效日期不优先于有效日期。最新公告已经展示过时不回退到旧公告；关闭、刷新或重新登录也不会继续排队弹出旧公告。发布新的最新公告后可再次弹出，原有的按账号保存已读记录继续生效。

弹框继续复用 `AnnouncementDetailModal`、共享 `Dialog` 和 `RichContent`，通过现有插槽增加标题栏、公告图标和确认按钮，采用方角边框与清晰的正文、附加信息分隔。发布时间和“知道了”位于固定底部，长正文在中间区域滚动；按钮、关闭叉和 Escape 均可关闭。看板内手动打开公告详情使用同一外观。新增按钮文案补齐七种语言，本次没有修改数据库或后端行为。

验证：`bun run test src/components/console-announcement-dialog src/features/dashboard/components/overview` 共 **2 个文件、18 项测试**通过，覆盖最新公告选择、已读和账号隔离、新公告更新、确认/关闭/Escape、附件、日期及滚动区域。`bun run typecheck`、三个变更文件的 oxlint、保留版权头的 oxfmt 检查、前端生产构建及 Linux 网关镜像构建通过。浏览器连接返回 `unsupported Codex auth method: apikey`，因此本轮未完成实机截图检查。验证日志保存在 `.local-tests/latest-announcement/`。
...
公告有稳定 ID 时按 ID 识别；没有 ID 时按内容和发布日期等字段识别。修改无 ID 公告的内容会视为新公告。首次使用此功能时，只展示最新且尚无账号已读记录的公告。
